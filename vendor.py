...
def add_alias(cur, raw_name, vendor_id):
    cur.execute("INSERT INTO vendor_aliases(raw_name, vendor_id) VALUES(?, ?)", (raw_name, vendor_id))
    
def get_vendor_id(cur, raw_name, vendor_name=None):
    vendor_id = find_alias(cur, raw_name)
    if vendor_id: 
        return vendor_id
    canonical_vendor_name = vendor_name or raw_name
    vendor_id = get_or_create_vendor(cur, canonical_vendor_name)
    add_alias(cur, raw_name, vendor_id)
    # con.commit()
    return vendor_id